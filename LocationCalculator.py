...
if __name__ == '__main__':
    ...
    with open('inputs/input100.csv', encoding='utf-8-sig') as csvfile:
        reader = csv.DictReader(csvfile, delimiter=',')
        mycsv = enumerate(reader, start=2)
        
        #create the set of ungrouped addresses
        ungrouped = []
        
        #get the first row set up
        first_row = reader.__next__()
        patent = first_row.get('patent_id')
        lat = first_row.get('inventor_add_lat')
        lng = first_row.get('inventor_add_lon')
        inventor_id = first_row.get('inventor_id')
        ...
        while row:
            ...
            try:
                ...
                break # make sure to unset to kill the loop
